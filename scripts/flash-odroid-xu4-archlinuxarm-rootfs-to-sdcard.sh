...
# Define variables
BUILD_ARCH="${BUILD_ARCH:-archlinux}"
ROOTFS_TARGZ="ArchLinuxARM-odroid-xu3-latest.tar.gz"
...
# Download ArchLinuxARM rootfs file
if [ ! -f "${ROOTFS_TARGZ}" ]; then
  wget "http://archlinuxarm.org/os/${ROOTFS_TARGZ}"
fi
...
${SUDO_CMD} bsdtar -xpf "${ROOTFS_TARGZ}" -C "${SDCARD_MOUNT}"
df -h "${SDCARD_DEVICE_NR}"
